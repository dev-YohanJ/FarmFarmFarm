$(document).ready(function () {
	...
	$('#idcheck').on('click', function () {
		...
		if($.trim($('#id').val()) == "") {
			...
			//첫 글자는 대문자이고 두번째부터는 대소문자, 숫자, _로 총 4개 이상
	...
	})// $('#idcheck').on('click', function(){}) end 
	
	//회원가입 버튼 클릭할 때 이벤트 처리 부분
	$("#myform").submit(function () {
		
		// $.trim(문자열)은 문자열의 앞, 뒤 공백을 제거합니다.
		if($.trim($('#id').val()) == "") {
			alert("ID를 입력하세요");
			$('#id').focus();
			return false;
		...
		if(!$("#id").prop('readOnly')){ // 회원가입 폼과 정보 수정 폼에서 동시에 사용할 js입니다.
										// 회원가입 폼에서만 사용할 문장들입니다.
										//정보 수정 폼에서는 아이디를 수정하지 않기 때문에 필요없는 부분입니다.
			
			
		console.log($("#id").prop('readOnly'));
		var submit_id_value = $.trim($('#id').val())
		if( submit_id_value != idcheck_value) {//submit 당시 아이디 값과 아이디 중복 검사에 사용된 아이디를 비교합니다.
			alert("ID 중복검사를 클릭하세요");	
			return false;
		}
		...
		/* 
			if(is_idcheck_click == false) {
				alert("ID 중복검사를 클릭하세요");	
				return false;
			} else if(idcheck_value != $.trim($('#id').val())) {
				alert("ID 중복검사를 클릭하세요");
				return false;			
			} 
		*/
		...
		var hobbys = $('input[name=hobby]:checkbox:checked');
		if(hobbys.length < 2) {
	...
	// 우편검색 버튼 클릭
	$('#postcode').on('click', function Postcode() {
        new daum.Postcode({
